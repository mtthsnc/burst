...
These apply throughout the pipeline. When in doubt, come back here.

- **Gaps over fabrication.** If something is unclear, say so. A PR that honestly notes "I'm unsure whether this covers RTL layouts" is more useful than one that silently ships without checking. Record what you skipped and why.
- **Scope is sacred.** Burst ships exactly what the user described. Not more, not less. No cleanup, no "while I'm here" refactors, no tests unless asked. If the change wants to grow, that's a signal to stop and check with the user.
- **Recommend, don't assume.** When the user's intent is ambiguous, propose your best read and let them correct it. Never fill in blanks silently.
...
If frontend files will be touched, you MUST have `agent-browser` available. Run `agent-browser --version` (or equivalent) to verify. If missing, ask the user for permission to install it before proceeding.
Do NOT proceed past this gate without all checks passing.
</HARD-GATE>
...
Gather context quickly - keep this shallow (a few file reads and commands, not deep analysis).
...
- Read CLAUDE.md / project instructions if present - respect repo-specific coding conventions
...
If the change touches more than ~3 files across unrelated concerns, that's a signal it may not be a burst. Say so: "This looks like it crosses X and Y - are you sure burst is the right mode, or should we plan this first?"
...
If the user's description was ambiguous, include your interpretation explicitly: "I'm reading this as X - correct me if you mean Y." Don't silently pick one reading and run with it.
...
After creating the issue, enrich its metadata. Query the repo for available issue types, labels, and other metadata fields, then set whatever fits the change:
...
gh api graphql -f query='{ repository(owner: "<org>", name: "<repo>") { issueTypes { nodes { id name } } } }'

# Set the best-fitting issue type (e.g. Bug, Task, Feature)
gh api graphql -f query='mutation { updateIssueIssueType(input: { issueId: "'"$ISSUE_ID"'", issueTypeId: "<chosen-type-id>" }) { issue { id } } }'
```

Best-effort only. If a mutation fails (missing token scope, field not available), log the error and move on - metadata is nice-to-have, not a gate.
...
- Bug is in the core ask itself: close the upstream issue with a comment explaining why the pipeline stopped, then report to the user. Do not open the PR.
...
git status  # verify only implementation files are staged
git commit -m "<type>(<scope>): <description>

Closes #<N>"
```

Stage only the files you intentionally changed. Do NOT use `git add -A` or `git add .` - these can pick up screenshots, temp files, or local configs.
...
```bash
gist_url=$(gh gist create before.png after.png -d "issue-<N> before/after")
...
## Test plan
- [x] <verified items with specifics>
- [ ] <items for human reviewer to check>

## Confidence
<one of: high / medium / low>. <one sentence explaining why - what you verified, what you couldn't.>
...
The **Confidence** section is your honest self-assessment. Examples:
- "High. Change is isolated to one SCSS rule, verified in browser at two viewports."
- "Medium. Logic change works in the happy path but I couldn't test the empty-cart edge case because the dev server doesn't seed that state."
- "Low. The fix addresses the symptom but the root cause might be upstream in the API response shape."
...
- What I'd check manually: <one line, or "nothing - fully verified">
```

If the change was scoped down from the original ask, say so here with a link to the follow-up issue.
...
| "I'll silently work around this" | If something surprised you mid-implementation, note it. Silent workarounds hide bugs. |
| "I'll fill in the blanks myself" | If the user's intent is ambiguous, you should have caught it in Step 2. If you missed it, stop and ask now rather than guessing. |
